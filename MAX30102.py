...
class MAX30102():
  ...
  # by default, thi, assumes that the device is at 0x57 on channel 1
  def __init__ (self, channel=1, address=0x57, gpio_pin=7):
    ...
    # print(.[SETUP] reset complete with interrupt register0: 01..format(reg_data)) 
    self.setup()
    # print(ISETUP] setup complete")
  ...
  def reset(self):
    ...
    # after running this, run setup() again.
  ...
  def setup(self, led_mode=0x03):
    ...
    # INTR setting
    # Oxc0  A_FULL_EN and PPG_RDY_EN = Interrupt will be triggered when
    ...
    # 0x02 for read-only, 0003 for Sp02 mode, 0x07 multimode LED 
    self.bus.write_i2c_block_data(self.address, REG_MODE_CONFIG, [led_mode]) 
    # Ob 0010 0111
    # SP02_ADC range = 4096nA, SP02 sample rate = 100Hz, LED pulse-wl = 411uS
    self.bus.write_i2c_block_data(self.address, REG_SPO2_CONFIG, [0x27])

    # choose value for -7mA for LED1
    self.bus.write_i2c_block_data(self.address, REG_LEDl_PA, [0x24]) 
 ...
 #use when changing the values from default
  ...
  def read_sequential(self, amount=100):
    #This function will read the red-led and it-led 'amount' times. 
